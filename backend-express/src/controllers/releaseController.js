...
const previewCertificate = async (req, res, next) => {
    try {
        ...
        try {
            ...
            const certData = {
                submissionId,
                institutionName: data.institusi,
                programName: data.program_studi,
...
const publishCertificate = async (req, res, next) => {
    try {
        ...
        try {
            ...
            const fileCid = `QmCert${Date.now()}-${uuidv4().substring(0,8)}`; 
            const fileHash = `0xHash${Date.now()}`;
            ...
            await client.query(`
                ...
                SET file_cid = EXCLUDED.file_cid, generated_at = CURRENT_TIMESTAMP
            `, [
                `CERT-${submissionId}`,
                submissionId,
                data.decision_id,
